...
eAreaD' :: forall t m a. MonadWidget t m => Dynamic t EArea -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eAreaD' b = elDynAttr' "area" (attrMap <$> b)

eAreaD :: forall t m a. MonadWidget t m => Dynamic t EArea -> m a -> m a
...
eAudioD' :: forall t m a. MonadWidget t m => Dynamic t EAudio -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eAudioD' b  = elDynAttr' "audio" (attrMap <$> b)

eAudioD :: forall t m a. MonadWidget t m => Dynamic t EAudio -> m a -> m a
eAudioD b children = snd <$> eAudioD' b children


eAudioC :: MonadWidget t m => EAudio -> m () -> m (Event t ())
eAudioC b children = do
  (e,_) <- elAttr' "audio" (attrMap b) children
  return $ domEvent Click e

eAudioCD :: MonadWidget t m => Dynamic t EAudio -> m () -> m (Event t ())
...
eCanvasD' :: forall t m a. MonadWidget t m => Dynamic t ECanvas -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eCanvasD' b = elDynAttr' "canvas" (attrMap <$> b)

eCanvasD :: forall t m a. MonadWidget t m => Dynamic t ECanvas -> m a -> m a
...
eIFrameD' :: forall t m a. MonadWidget t m => Dynamic t EIFrame -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eIFrameD' b  = elDynAttr' "iframe" (attrMap <$> b)

eIFrameD :: forall t m a. MonadWidget t m => Dynamic t EIFrame -> m a -> m a
eIFrameD b children = snd <$> eIFrameD' b children


eIFrameC :: MonadWidget t m => EIFrame -> m () -> m (Event t ())
eIFrameC b children = do
  (e,_) <- elAttr' "iframe" (attrMap b) children
  return $ domEvent Click e

eIFrameCD :: MonadWidget t m => Dynamic t EIFrame -> m () -> m (Event t ())
...
eImgD' :: forall t m a. MonadWidget t m => Dynamic t EImg -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eImgD' b  = elDynAttr' "img" (attrMap <$> b)

eImgD :: forall t m a. MonadWidget t m => Dynamic t EImg -> m a -> m a
eImgD b children = snd <$> eImgD' b children


eImgC :: MonadWidget t m => EImg -> m () -> m (Event t ())
eImgC b children = do
  (e,_) <- elAttr' "img" (attrMap b) children
  return $ domEvent Click e

eImgCD :: MonadWidget t m => Dynamic t EImg -> m () -> m (Event t ())
...
eMapD' :: forall t m a. MonadWidget t m => Dynamic t EMap -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eMapD' b = elDynAttr' "map" (attrMap <$> b)

eMapD :: forall t m a. MonadWidget t m => Dynamic t EMap -> m a -> m a
...
eObjectD' :: forall t m a. MonadWidget t m => Dynamic t EObject -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eObjectD' b  = elDynAttr' "object" (attrMap <$> b)

eObjectD :: forall t m a. MonadWidget t m => Dynamic t EObject -> m a -> m a
...
eParamD' :: forall t m a. MonadWidget t m => Dynamic t EParam -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eParamD' b  = elDynAttr' "param" (attrMap <$> b)

eParamD :: forall t m a. MonadWidget t m => Dynamic t EParam -> m a -> m a
...
ePictureD' :: forall t m a. MonadWidget t m => Dynamic t EPicture -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
ePictureD' b  = elDynAttr' "picture" (attrMap <$> b)

ePictureD :: forall t m a. MonadWidget t m => Dynamic t EPicture -> m a -> m a
...
eSourceD' :: forall t m a. MonadWidget t m => Dynamic t ESource -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eSourceD' b  = elDynAttr' "source" (attrMap <$> b)

eSourceD :: forall t m a. MonadWidget t m => Dynamic t ESource -> m a -> m a
...
eTrackD' :: forall t m a. MonadWidget t m => Dynamic t ETrack -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eTrackD' b  = elDynAttr' "track" (attrMap <$> b)

eTrackD :: forall t m a. MonadWidget t m => Dynamic t ETrack -> m a -> m a
...
eVideoD' :: forall t m a. MonadWidget t m => Dynamic t EVideo -> m a -> m (Element EventResult (DomBuilderSpace m) t, a)
eVideoD' b  = elDynAttr' "video" (attrMap <$> b)

eVideoD :: forall t m a. MonadWidget t m => Dynamic t EVideo -> m a -> m a
eVideoD b children = snd <$> eVideoD' b children


eVideoC :: MonadWidget t m => EVideo -> m () -> m (Event t ())
eVideoC b children = do
  (e,_) <- elAttr' "video" (attrMap b) children
  return $ domEvent Click e

eVideoCD :: MonadWidget t m => Dynamic t EVideo -> m () -> m (Event t ())
